    ...
    if (isset($_POST['verify_payment'])) {
        ...
        if ($stmt) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                // ——— ONLY THIS PART WAS CHANGED (your exact request) ———
                ...
                if ($should_verify_now) {
                    $update_verify_query = "UPDATE users SET verify = 1, verify_time = ? WHERE email = ?";
                    $update_stmt = mysqli_prepare($con, $update_verify_query);
                    if ($update_stmt) {
                        mysqli_stmt_bind_param($update_stmt, "ss", $created_at, $email);
                        mysqli_stmt_execute($update_stmt);
                        mysqli_stmt_close($update_stmt);
                    }
                    $_SESSION['success'] = $payment_plan == 1 
                        ? "Payment submitted successfully! Your account is now verified."
                ...
                error_log("verify-complete.php - " . ($should_verify_now ? "Account AUTO-VERIFIED" : "Payment recorded") . " for email: $email | Plan: $payment_plan | Installment: $installment_number");
                // ————————————————————————————————————————————————
...
<!-- The rest of your HTML/PHP (completely unchanged) -->
                ...
                <div class="col-md-8">
                    <div class="card text-center">
                        ...
                        <div class="card-body mt-2">
                            ...
                            ?>
                                <!-- Your existing payment display code remains 100% unchanged -->
                                <!-- ... (all your original HTML here) ... -->
                                <!-- Form and everything else exactly as you had it -->
                            <?php } else { ?>
                                <p>No payment details available for your country. Please contact support.</p>
                                <?php error_log("verify-complete.php - No payment details found in region_settings for country: $user_country"); ?>
                            <?php } ?>
                        </div>
                    </div>
                </div>
